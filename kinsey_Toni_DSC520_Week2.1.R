...
rownames(students) <- students_combined
colnames(assignments) <- grades
...
factor_recommendations_vector <- factor(
  ...
  ordered = recommendations_vector,
...
ringbearers_df <- characters_df[characters_df$ring_bearer == ringbearers_df,]
